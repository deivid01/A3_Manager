import { CalendarDays, CheckCircle2, ChevronRight, Filter, Package, ReceiptText, RotateCcw, Search, SearchX, X } from "lucide-react";
...
export function RentalsView() {
  ...
  return (
    <section className="view" data-screen="rentals">
      ...
      <section className="rental-results" aria-busy={loading}>
        ...
        {loading && result.rows.length === 0 ? (
          ...
          <div className="rental-list">
            {result.rows.map((rental) => <RentalRow key={rental.id} rental={rental} onOpen={() => void openDetail(rental.id)} />)}
          </div>
        )}
        {hasMore && (
          <AppButton
      ...
      {selected && <RentalDetailModal rental={selected} onClose={() => setSelected(null)} onFinalize={() => setFinalizeConfirm(true)} />}
...
function RentalRow({ rental, onOpen }: { rental: RentalListItem; onOpen(): void }) {
  return (
